...
class ProductComponentsImportController extends Controller
{
        ...
        if (($result['accepted'] ?? false) === true) {
            try {
                $latestFile = $importFiles->replaceLatest(
                    ImportFileService::TYPE_PRODUCT_COMPONENTS,
                    $request->file('file'),
                    $request->user()?->id
                );
            } catch (\Throwable $exception) {
                Log::warning('product_components_import_file_store_failed', [
    ...
    {
        $latestFile = $importFiles->latestMetadata(ImportFileService::TYPE_PRODUCT_COMPONENTS);
    ...
    {
        return $importFiles->downloadLatest(ImportFileService::TYPE_PRODUCT_COMPONENTS);
    }
}
